         ...
         inContext:(CGContextRef)context
{
	...
	if (!self.theme.sliceDividerHidden && self.theme.sliceDividerThickness > 0) {
        ...
        CGFloat sliceAngle = (2 * self.endingAngle ) / slicesCount;
        for (int i =0; i < slicesCount; i++) {
            CGFloat startValue = (sliceAngle * i) + sliceAngle * startingSlice;
            CGFloat startAngle, endAngle;
            if (self.clockwise) {
                startAngle = - self.endingAngle + startValue;
		...
		CGFloat sliceAngle = (2 * self.endingAngle) / self.progressTotal;
		...
		if (self.progressCounter == 0) {
			originAngle = -self.endingAngle;
			endAngle = originAngle + 2 * self.endingAngle;
		} else {
			if (self.clockwise) {
				originAngle = -self.endingAngle + startingAngle;
...
//			endAngle = originAngle;
            ...
            if (self.progressCounter > 0) {
                CGContextAddArc(context, center.x, center.y, circleRadius, endAngle, self.endingAngle, cgClockwise);
            }else{
                CGContextAddArc(context, center.x, center.y, circleRadius, originAngle, self.endingAngle, cgClockwise);
            }
...
- (void)drawSlicesSeparators:(CGContextRef)contextRef withViewSize:(CGSize)viewSize andCenter:(CGPoint)center
{
    ...
    if (! self.theme.sliceDividerHidden) {
        ...
        float sliceAngle = (2 * self.endingAngle) / sliceCount;
        ...
        for (int i = 0; i < sliceCount; i++) {
            double startAngle = sliceAngle * i - self.endingAngle;
			double endAngle = sliceAngle * (i + 1) - self.endingAngle;
