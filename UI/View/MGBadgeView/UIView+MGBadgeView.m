...
- (void)drawRect:(CGRect)rect {
    ...
    if(_badgeValue != 0 || _displayIfZero) {
        ...
        NSString *stringToDraw = _badgeValue;
        ...
        if (_outlineWidth>0) {
            [outlinePath setLineWidth:_outlineWidth];
...
- (void)setBadgeValue:(NSString *)badgeValue{
    ...
    if(_badgeValue != badgeValue) {
        ...
        if((badgeValue.length>0 && ![badgeValue isEqualToString:@"0"]) || _displayIfZero) {
...
- (void)mg_updateBadgeViewSize {
    ...
    CGSize numberSize = [_badgeValue sizeWithAttributes:@{NSFontAttributeName: _font}];
    
    float badgeHeight = MAX(BADGE_TOTAL_OFFSET + numberSize.height, _minDiameter);
    float badgeWidth = MAX(badgeHeight, BADGE_WIDTH_OFFSET + numberSize.width);
...
- (void)mg_updateBadgeViewPosition {
    ...
    switch (position) {
        ...
        case MGBadgePositionCenterLeft: {
            self.center = CGPointMake(0, superviewFrame.size.height/2);
            break;
        }
        case MGBadgePositionCenterRight: {
            self.center = CGPointMake(superviewFrame.size.width, superviewFrame.size.height/2);
            break;
        }
