...
- (id)initWithTitle:(NSString *)title buttons:(NSArray *)buttons delegate:(id<LXActionSheetDelegate>)delegate
{
    ...
    if (self) {
        ...
        [self creatButtonsWithTitle:title buttons:buttons];
...
- (void)creatButtonsWithTitle:(NSString *)title buttons:(NSArray *)buttons
{
    //初始化
    self.isHadTitle = NO;
    ...
    if (buttons&&buttons.count>0) {
        ...
        if (self.isHadTitle == NO) {
            for (int i = 0; i<buttons.count; i++) {
                UIButton *otherButton = [buttons objectAtIndex:i];
                ...
                if (i==0) {
                    ...
                    frame.origin.y = BUTTON_INTERVAL_HEIGHT;
                    otherButton.frame = frame;
                    self.LXActionSheetHeight+= BUTTON_INTERVAL_HEIGHT;
                }
