...
- (instancetype)initWithName:(NSString *)name
{
    ...
    if (self = [super init]) {
        ...
        dispatch_async(_queue, ^{
            ...
            [NSFileManager addSkipBackupAttributeToItemAtURL:_cacheURL];
...
- (instancetype)initWithDirectory:(NSSearchPathDirectory)directory andPath:(NSString*)path
{
    if (!path || !directory)
        return nil;
    ...
    if (self = [super init]) {
        ...
        NSString *pathComponent = [[NSString alloc] initWithFormat:@"%@", _name];
        _cacheURL = [NSURL fileURLWithPathComponents:@[ [paths objectAtIndex:0], pathComponent ]];
